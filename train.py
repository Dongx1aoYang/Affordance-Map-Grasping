...
def get_center_angle(
    ...
    dy = right_coord[1]-left_coord[1]
    dx = right_coord[0]-left_coord[0]
...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx: int
        ...
        # Get finger poiot on the orignial rgb img
        left, right = get_finger_points(center_point, angle)
        # Mark the key points
        kpt = KeypointsOnImage([Keypoint(x=left[0],y=left[1]), Keypoint(x=right[0],y=right[1])], shape=rgb.shape)
        # Augment data
        img_aug, kpt_aug = self.aug_pipeline(image=rgb.numpy(), keypoints=kpt)
        # Get center point and angle after augmentation
        left_aug = np.array([kpt_aug.keypoints[0].x, kpt_aug.keypoints[0].y])
        right_aug = np.array([kpt_aug.keypoints[1].x, kpt_aug.keypoints[1].y])
        center_point_aug, angle_aug = get_center_angle(left_aug, right_aug)
        data = {
            'rgb': img_aug,
            'center_point': center_point_aug,
            'angle': np.array(angle_aug)
        }
        for key, value in data.items():
            data_torch[key] = torch.from_numpy(value)
        return data_torch
