...
class ActionRegressionDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # ===============================================================================
        # Normalize and permute the input image
        rgb = data['rgb'] # torch 128,128,3 uint8      
        input = (rgb/255).float().permute(2,0,1)

        # Normalize center point and angle
        center_point = data['center_point'] # torch.tensor float
        angle = data['angle'] # torch.float

        #max_x = rgb.shape[1]
        #max_y = rgb.shape[0]
        #normalized_center_point = (center_point[0]/max_x, center_point[1]/max_y)

        normalized_center_point = ((center_point[0]-25)/75, (center_point[1]-30)/60)
        normalized_angle = (angle+180)/360
        
        target = torch.tensor([normalized_center_point[0], normalized_center_point[1], normalized_angle], dtype=torch.float32)
...
def recover_action(
    ...
    #max_x = shape[1]
    #max_y = shape[0]
    #coord = (np.round(raw_coord[0]*max_x).astype(int), np.round(raw_coord[1]*max_y).astype(int))
    coord = (np.round(raw_coord[0]*75+25).astype(int), np.round(raw_coord[1]*60+30).astype(int))
    angle = action[-1]*360-180
    # ===============================================================================
    return coord, angle
...
class ActionRegressionModel(nn.Module):
    ...
    def predict_grasp(self, rgb_obs: np.ndarray
        ...
        # ===============================================================================
        coord, angle = None, None
        # Modify the rgb_obs (128, 128, 3 numpy) to fit the input type torch.Tensor (3,128,128), torch.float32
        rgb_input = torch.tensor(rgb_obs/255, dtype=torch.float32).permute(2,0,1).unsqueeze(0)
        rgb_input_device = rgb_input.to(device)
        
        pred = self.predict(rgb_input_device)
        
        # Convert to cpu and numpy
        raw_coord_device = pred[0,:2]
        raw_angle_device = pred[0,-1]

        raw_coord = raw_coord_device.cpu().detach().numpy()
        raw_angle = raw_angle_device.cpu().detach().numpy()

        raw_action = np.array([raw_coord[0], raw_coord[1], raw_angle]).flatten()
        # recover the action
        coord, angle = recover_action(raw_action)

        # ===============================================================================
        # visualization
        vis_rgb = np.moveaxis(rgb_obs,-1,0)
        vis_img = self.visualize(vis_rgb, raw_action)
        return coord, angle, vis_img
